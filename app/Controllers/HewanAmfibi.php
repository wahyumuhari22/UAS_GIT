...
use App\Models\ModelHewanAmfibi;

class HewanAmfibi extends BaseController
{
    public function index()
    {
        return view('HewanAmfibi/tampilhewan');
    }

    public function ambildata()
    {
        if ($this->request->isAJAX()) {
            $mhs = new ModelHewanAmfibi();
            $data = [
                'tampildata' => $mhs->findAll()
            ];
            $msg = [
                'data' => view('HewanAmfibi/datahewan', $data) // Tambahkan $ sebelum data
            ];
    ...
    public function formtambah()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'data' => view('HewanAmfibi/tambahhewan', $data)
            ];
            echo json_encode($msg);
        } else {
            exit('maaf tidak dapat diproses');
    ...
    public function simpandata()
    {
        if ($this->request->isAJAX()) {
            ...
            $valid = $this->validate([
                'id' => [
                    'label' => 'ID',
                    'rules' => 'required|is_unique[hewan_amfibi.id]',
                    'errors' => [
            ...
            if (!$valid) {
                ...
                $msg = [
                    'success' => 'Data HewanAmfibi berhasil disimpan'
                ];
                echo json_encode($msg);
            }
        } else {
    ...
    public function formedit()
    {
        if ($this->request->isAJAX()) {
            ...
            $data = [
                // sebelah kanan field pada tabel HewanAmfibi
            ...
            $msg = [
                'sukses' => view('HewanAmfibi/modalhewan', $data)
            ];
